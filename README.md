...
也可以手动锁定浅色 / 深色，锁定后不再自动变。切换时同步更新 `theme-color`，iOS Safari 的顶栏与底部工具栏颜色会跟着走。
...
以 **iOS Safari** 与 **Chrome / Edge / Safari 桌面版** 为主力目标，针对移动端 WebKit 的滚动回弹、工具栏收展、bfcache 恢复做了专门处理。
